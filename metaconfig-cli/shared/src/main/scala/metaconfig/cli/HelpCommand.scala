...
class HelpCommand(
  ...
  override def complete(
    ...
    if (context.arguments.length <= 1) context.app.commands.filterNot(_.isHidden)
      .map(c => TabCompletionItem(c.name))
    else Nil
  def run(options: HelpOptions, app: CliApp): Int = options.subcommand match {
    case Nil =>
      ...
      0
    case subcommand :: Nil => app.commands.find(_.matchesName(subcommand)) match {
        case Some(command) =>
          command.helpMessage(app.out, screenWidth)
          0
        case None => notRecognized(subcommand, app)
      }
    case obtained =>
      app.error(
  ...
  def notRecognized(subcommand: String, app: CliApp): Int = {
    ...
    app.error(s"no such subcommand '$subcommand'.$didYouMean\n\tTry '${app.binaryName} help' for more information.")
    1
  }

  val noSubcommand: Command[Unit] = this.contramap[Unit]((), _ => HelpOptions())
}
