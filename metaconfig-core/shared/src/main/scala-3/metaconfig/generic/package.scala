...
inline def deriveEncoder[T]: metaconfig.ConfEncoder[T] = ${ deriveEncoderImpl[T] }
...
inline def deriveDecoderEx[T](
    inline default: T,
): metaconfig.ConfDecoderExT[T, T] = ${ deriveConfDecoderExImpl[T]('default) }

inline def deriveSurface[T]: Surface[T] = ${ deriveSurfaceImpl[T] }
  ...
  '{
    new metaconfig.ConfEncoder[T]:
      override def write(value: T): metaconfig.Conf =
        ...
        new metaconfig.Conf.Obj($fields.zip($encoders).zipWithIndex.map {
          case ((name, enc), idx) => name ->
              enc.asInstanceOf[ConfEncoder[Any]].write(prod.productElement(idx))
        })
  }
...
private[generic] def deriveConfDecoderImpl[T: Type](default: Expr[T])(using
  ...
  def next(p: ValDef): Expr[Conf => Configured[Any]] = p.tpt.tpe.asType match
    case '[t] =>
...
private[generic] def deriveConfDecoderExImpl[T: Type](default: Expr[T])(using
  ...
  def next(p: ValDef): Expr[(Conf, T) => Configured[Any]] =
    p.tpt.tpe.asType match
      case '[t] =>
        ...
        val getter = clsTpt.classSymbol.get.declaredField(p.name)
        val fallback =
          (from: Expr[Any]) => Select(from.asTerm, getter).asExprOf[t]
  ...
  if paramss.head.isEmpty then
    '{
      new ConfDecoderEx[T]:
        def read(state: Option[T], conf: Conf) = Configured
...
private[generic] def deriveSurfaceImpl[T: Type](using q: Quotes) =
  ...
  val argss = cls.primaryConstructor.paramSymss
    .filter(paramList => paramList.forall(!_.isTypeParam)).map { params =>
      val fields = params.flatMap { param =>
        param.tree match
          case vd: ValDef =>
            inline def derivesFrom[A: Type]: Boolean = vd.tpt.tpe
              .derivesFrom(TypeRepr.of[A].typeSymbol)

            val baseAnnots = param.annotations.collect {
              case annot
                  if annot.tpe
                    .derivesFrom(TypeRepr.of[StaticAnnotation].typeSymbol) =>
            ...
            val tpeString = fieldType.asType match
              case '[t] =>
  ...
  val classAnnotations = Expr.ofList {
    cls.annotations.collect {
      case annot
          if annot.tpe.derivesFrom(TypeRepr.of[StaticAnnotation].typeSymbol) =>
        annot.asExprOf[StaticAnnotation]
    }
  }
  ...
  Expr.ofList {
    fields.map { f =>
      ...
      Implicits.search(encoder.appliedTo(tp)) match
        case iss: ImplicitSearchSuccess => iss.tree.asExpr
