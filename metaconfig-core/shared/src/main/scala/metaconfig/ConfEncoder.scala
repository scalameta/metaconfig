...
trait ConfEncoder[A] { self =>
  ...
  final def writeObj(value: A): Conf.Obj = write(value) match {
    case o: Conf.Obj => o
...
object ConfEncoder {
  ...
  private val GenericConfEncoder: ConfEncoder[Conf] = instance(identity)

  // Invariant type-classes don't work unfortunately so we do this work by hand.
  implicit def ConfEncoder[T <: Conf]: ConfEncoder[T] = GenericConfEncoder
    .asInstanceOf[ConfEncoder[T]]

  implicit val BooleanEncoder: ConfEncoder[Boolean] = Conf.Bool(_)

  implicit val IntEncoder: ConfEncoder[Int] = Conf.Num(_)

  implicit val UnitEncoder: ConfEncoder[Unit] = _ => Conf.Null()
  ...
  implicit def OptionEncoder[A](implicit
      ev: ConfEncoder[A],
  ): ConfEncoder[Option[A]] = _.fold[Conf](Conf.Null())(ev.write)
