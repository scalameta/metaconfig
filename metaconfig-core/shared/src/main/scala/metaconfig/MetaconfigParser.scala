...
trait MetaconfigParser {
  ...
  def parseFilename[T: ConfDecoder](
      filename: String,
      text: String,
  ): Configured[T] = parseInput(Input.VirtualFile(filename, text))
  def parseInput[T: ConfDecoder](input: Input): Configured[T] = fromInput(input)
    .andThen(ConfDecoder[T].read)
  ...
  final def fromFile(path: Path): Configured[Conf] = fromInput(Input.File(path))
}
