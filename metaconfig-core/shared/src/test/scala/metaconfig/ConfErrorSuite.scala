...
class ConfErrorSuite extends FunSuite {
  ...
  // Located here to minimize changing line number
  check(
    "exception",
    exception(new IllegalArgumentException("Bad!"), 1),
    """|java.lang.IllegalArgumentException: Bad!
       |	at metaconfig.ConfErrorSuite$$anonfun$1.apply(ConfErrorSuite.scala:14)""".stripMargin
  )
