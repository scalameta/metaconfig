...
class BaseCliParserSuite extends munit.FunSuite {
  ...
  def toString(options: Options): String = settings.settings
    .zip(options.productIterator.toList).map { case (s, v) =>
      s"${s.name} = $v"
    }.mkString("\n")
  def check(
      name: String,
  ...
  def checkError(name: String, args: List[String], expected: String): Unit =
    test(name) {
      val options = Conf.parseCliArgs[Options](args).andThen(_.as[Options])
