...
class CliParserSuite extends BaseCliParserSuite {
  ...
  check("boolean", "--clean-target" :: Nil, Options(cleanTarget = true))
  ...
  check("string", "--in" :: "in" :: Nil, Options(in = "in"))
  ...
  check("kebab", "--base-url" :: "base-url" :: Nil, Options(baseUrl = "base-url"))

  check("remainingArgs", "arg1" :: Nil, Options(remainingArgs = "arg1" :: Nil))
  ...
  check(
    "map2",
    "--site.custom.key1" :: "value1" :: "--site.custom.key2" :: "value2" :: Nil,
    Options(site = Site(custom = Map("key1" -> "value1", "key2" -> "value2"))),
  )

  check(
    "mapCustom2",
    "--site.custom2.key1" :: "value1" :: "--site.custom2.key2" :: "value2" ::
      "--site.custom2.key3" :: "value3" :: "--site.custom2.key4" :: "value4" ::
      "--site.custom2.key5" :: "value5" :: Nil,
    Options(site =
      Site(custom2 =
        ListMap(
          "key1" -> "value1",
  ...
  check(
    "inline",
    "--foo" :: "blah" :: "--inlined.custom.explicit" :: "boom" ::
      "--custom.bar" :: "buz" :: Nil,
    Options(inlined =
      Site(foo = "blah", custom = Map("bar" -> "buz", "explicit" -> "boom")),
    ),
  )

  check("=", "--title=buzz" :: Nil, Options(title = "buzz"))
  ...
  check(
    ...
    Options(conf =
      Conf.Obj("x" -> Conf.Obj("y" -> Conf.Str("true")), "z" -> Conf.Str("1.0")),
    ),
  )
  ...
  check(
    ...
    Options(title = "buzz", remainingArgs = List("positional1", "positional2")),
  )

  check(
    "positional-everywhere3",
    "positional1" :: "--title" :: "buzz" :: Nil,
    Options(title = "buzz", remainingArgs = List("positional1")),
  )
