...
class SubcommandSuite extends FunSuite {
  case class CommonOptions(beep: Boolean = false)
  ...
  implicit val commonSurface: Surface[CommonOptions] = metaconfig.generic
    .deriveSurface[CommonOptions]
  implicit val commonCodec: ConfCodec[CommonOptions] = metaconfig.generic
    .deriveCodec[CommonOptions](CommonOptions())
  implicit val surface: Surface[TestOptions] = metaconfig.generic
    .deriveSurface[TestOptions]
  implicit val codec: ConfCodec[TestOptions] = metaconfig.generic
    .deriveCodec[TestOptions](TestOptions())
  object TestCommand extends Command[TestOptions]("test") {
    ...
    override def examples: Doc = Doc
      .text("app test --max-count=100 project-name")
  ...
  def check(args: List[String], expected: String, expectedExit: Int = 0)(
      implicit loc: munit.Location,
  ): Unit = test(args.mkString(" ")) {
