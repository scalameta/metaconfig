...
class DeriveConfDecoderExSuite extends munit.FunSuite {
  ...
  test("iterable") {
    val obtained = IsIterable.decoderEx
      .read(None, Obj("b" -> Conf.Lst(Conf.Str("33")))).get
    assert(obtained == IsIterable(b = Iterable("33")))
  }

  test("one param") {
    val obtained = OneParam.decoderEx
      .read(Some(OneParam(42)), Obj("param" -> Num(2))).get
  ...
  test("either") {
    implicit val decoderOneParam: ConfDecoderEx[OneParam] = generic
      .deriveDecoderEx[OneParam](OneParam()).noTypos
    implicit val decoderHasOption: ConfDecoderEx[HasOption] = generic
      .deriveDecoderEx[HasOption](HasOption()).noTypos
    val either = implicitly[ConfDecoderEx[Either[OneParam, HasOption]]]
    assertEquals(either.read(None, Obj("param" -> Num(2))).get, Left(OneParam(2)))
    assertEquals(
  ...
  def checkOption(conf: Conf, expected: HasOption, state: HasOption = HasOption())(
      implicit decoder: ConfDecoderEx[HasOption],
  ): Unit = test("option-" + conf.toString) {
