...
class DeriveConfDecoderSuite extends munit.FunSuite {
  ...
  def checkOk(name: String, obj: Conf, expected: AllTheAnnotations): Unit =
    test(name) {
      ConfDecoder[AllTheAnnotations].read(obj) match {
        ...
        case Configured.Ok(obtained) => assert(obtained == expected)
      }
    }
  ...
  test("iterable") {
    val obtained = ConfDecoder[IsIterable]
      .read(Obj("b" -> Conf.Lst(Conf.Str("33")))).get
  ...
  test("either") {
    implicit val decoderOneParam: ConfDecoder[OneParam] = generic
      .deriveDecoder[OneParam](OneParam()).noTypos
    implicit val decoderHasOption: ConfDecoder[HasOption] = generic
      .deriveDecoder[HasOption](HasOption()).noTypos
