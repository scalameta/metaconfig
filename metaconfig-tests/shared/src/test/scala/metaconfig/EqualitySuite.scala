...
class EqualitySuite extends munit.FunSuite {
  def checkNotEqual(a: Conf, b: Conf): Unit =
    test("ne " + a.toString)(assertNotEquals(a, b))

  def checkEqual(a: Conf, b: Conf): Unit =
    test("eq " + a.toString)(assertEquals(a, b))

  checkNotEqual(Conf.Lst(Conf.Str("a"), Conf.Str("b")), Conf.Lst(Conf.Str("a")))
  ...
  checkNotEqual(Conf.Str("b"), Conf.Str("c"))
