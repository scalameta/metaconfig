...
trait ConfCodec[A] extends ConfDecoder[A] with ConfEncoder[A] { self =>
  def bimap[B](in: B => A, out: A => B): ConfCodec[B] = new ConfCodec[B] {
    override def write(value: B): Conf =
      self.write(in(value))
    override def read(conf: DecoderContext): Configured[B] =
      self.read(conf).map(out)
  }
}
...
object ConfCodec {
  ...
  implicit def EncoderDecoderToCodec[A](
    ...
    override def read(conf: DecoderContext): Configured[A] = decode.read(conf)
  }
